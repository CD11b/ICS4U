...
pygame.display.set_caption("ReRun")
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if shopButton2.isOver(pos):  # if the player exits the shop, it brings them to the next level
                    ...
                    level_list[1] = pygame.image.load(
                        "EnvImages/level_" + str(level_list[0]) + ".png")  # Level image sent to list
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:  # if the event type is click buttons
                if greyButton3.isOver(pos):
                    ...
                    if player_gold[0] - 250 >= 0:  # checks if the player has enough money to buy the weapon
                        ...
                        message_to_screen("NOT ENOUGH COINS", red, 600, 300,
                                          "medium")  # informs the player if they don't have enough coins
                        pygame.display.update()
                        nocoinsound.play()
                        time.sleep(1)
                        pygame.display.update()
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:  # if even type is clicking buttons
                if greyButton3.isOver(pos):
                    ...
                    if player_gold[0] - 250 >= 0:
                        ...
                        message_to_screen("NOT ENOUGH COINS", red, 600, 300,
                                          "medium")  # tells the player they don't have enough coins
                        pygame.display.update()
                        time.sleep(1)
                        pygame.display.update()
                        nocoinsound.play()
...
class Character(pygame.sprite.Sprite):
    def __init__(self, x, y, width, height, name):
        ...
        self.lookUp = pygame.image.load("EnvImages/" + self.name + '90.png')
        self.lookDown = pygame.image.load("EnvImages/" + self.name + '270.png')
        self.lookLeft = pygame.image.load("EnvImages/" + self.name + '180.png')
        self.lookRight = pygame.image.load("EnvImages/" + self.name + '0.png')
        self.lookIdle = pygame.image.load("EnvImages/" + self.name + '0.png')
...
if level_list[0] == 1:  # If level 1
    ...
    level_list[1] = pygame.image.load("EnvImages/level_1.png")  # Level image sent to list

player_gold = [0]


def redraw():  # Function used to draw to the screen
    if level_list[0] == 1 and len(
            guards_killed) >= 6:  # If player is on the first level and has cleared all of the enemies
...
def game_loop():
    ...
    while run:  # While the game is active/running. If run ever = False, end the game.
        ...
        if len(wardens) == 0 and level_list[0] == 5 and len(
                wardens_killed) < 1:  # If the player is on level 3, start adding wardens to the screen
